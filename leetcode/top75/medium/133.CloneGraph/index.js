...
/** Solution: BFS
 *  TC: O(N + E) - We visit each node and edge once
 *  SC: O(N) - The visited map stores a clone for each node
 */
var cloneGraph = function (node) {
  ...
  // Create the clone for the start node and put into visited
  ...
  while (queue.length > 0) {
    ...
    for (const nei of cur.neighbors) {
      ...
      curClone.neighbors.push(visited.get(nei));
    }
  }
...
var cloneGraph2 = function (node) {
  ...
  function dfs(n) {
    ...
    for (const nei of n.neighbors) {
      const neiClone = dfs(nei);   // recursively clone neighbour
      copy.neighbors.push(neiClone);
    }
